...
| User Story   | US9  | Integrar pasarela de pago (Stripe) | Como empresa, quiero integrar Stripe para cobrar suscripciones de manera segura. | Given un cliente selecciona un plan<br>When confirma el pago<br>Then Stripe procesa la transacción y el sistema la registra como exitosa o rechazada. | E5 |
| User Story   | US10 | Crear suscripción | Como cliente final, quiero contratar una suscripción, para acceder a las funcionalidades del sistema. | Given un cliente sin suscripción activa<br>When elige un plan y lo paga<br>Then el sistema crea la suscripción y la marca como activa. | E5 |
| User Story   | US11 | Cancelar suscripción | Como cliente final, quiero cancelar mi suscripción, para detener los cobros futuros. | Given un cliente con suscripción activa<br>When solicita la cancelación<br>Then el sistema marca la suscripción como cancelada y detiene próximos pagos. | E5 |
| User Story   | US12 | Habilitar/Inhabilitar uso de aplicación | Como empresa, quiero que solo los clientes con suscripción activa puedan usar la aplicación. | Given un cliente con suscripción activa<br>When inicia sesión<br>Then el sistema le permite acceder.<br><br>Given un cliente con suscripción inactiva<br>When intenta iniciar sesión<br>Then el sistema restringe el acceso y muestra mensaje de renovación. | E5 |
| User Story   | US13 | Visualizar información de suscripción | Como cliente final, quiero ver mi estado de suscripción y fecha de expiración, para gestionar mi acceso al servicio. | Given un cliente autenticado accede a la sección de suscripción<br>When consulta su información<br>Then el sistema muestra el plan, estado y fecha de expiración. | E5 |
| User Story   | US14 | Historial de pagos | Como empresa, quiero que los clientes consulten su historial de pagos, para brindar transparencia. | Given un cliente autenticado<br>When accede al historial de pagos<br>Then el sistema muestra todas las transacciones registradas con fecha y monto. | E5 |
| User Story   | US15 | Notificación de renovación próxima | Como cliente final, quiero recibir una notificación antes de que mi suscripción se renueve, para decidir si continúo o cancelo. | Given una suscripción está próxima a renovarse (ej. 3 días antes)<br>When el sistema procesa la fecha<br>Then envía una notificación al cliente. | E5 |
| User Story   | US16 | Cambio de plan de suscripción | Como cliente final, quiero cambiar mi plan de suscripción, para ajustarlo a mis necesidades. | Given un cliente con plan activo<br>When solicita un cambio de plan<br>Then el sistema ajusta la suscripción y actualiza el cobro. | E5 |
| User Story   | US17 | Reembolso de cobro inválido | Como empresa, quiero gestionar reembolsos cuando ocurre un cobro duplicado o fallido, para mantener confianza con los clientes. | Given un pago se marca como inválido<br>When el administrador aprueba el reembolso<br>Then el sistema procesa la devolución a través de Stripe. | E5 |
| User Story   | US18 | Auditoría de suscripciones | Como administrador, quiero registrar todas las acciones sobre suscripciones, para fines de control y trazabilidad. | Given se crea, modifica o cancela una suscripción<br>When se guarda el evento<br>Then el sistema registra la acción en el log de auditoría. | E5 |
